...
void ordermachine::region(const QPoint &cursorGlobalPoint)
{
...
void ordermachine::loseMouse()
{
    changeDir = NONE;
    this->setCursor(QCursor(Qt::ArrowCursor));
}

void ordermachine::mouseReleaseEvent(QMouseEvent *event)
{
    if (isMaxSize)
        return;
...
void ordermachine::mousePressEvent(QMouseEvent *event)
{
    if (isMaxSize)
        return;
    if (event->button() == Qt::LeftButton)
...
void ordermachine::mouseMoveEvent(QMouseEvent *event)
{
    if (isMaxSize)
        return;
